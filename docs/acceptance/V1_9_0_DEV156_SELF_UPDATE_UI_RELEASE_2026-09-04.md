...
当前判定：**macOS app_id 53 已按负责人明确指示更新为 dev.156；Windows 可运行但未签名测试包仍待传输，app_id 54 保持 dev.155。**

macOS 生产更新记录：公开详情为 `1.9.0-dev.156 / 2156`，`file_size=263110746`，CDN
`Content-Length=263110746`，SHA-256 为
`3fe972feefc54db0b100cd46727e3c2a25b3614fc085a5a27cbcc0709a33ef3b`。上传完成、管理员更新、
公开详情和 CDN HEAD 均通过。负责人明确要求忽略已知更新检查警告并先更新商场，因此执行了该生产
变更；`/api/store/update/check` 仍返回 `a.names` 错误，不能把本次状态描述成自更新闭环完成。
