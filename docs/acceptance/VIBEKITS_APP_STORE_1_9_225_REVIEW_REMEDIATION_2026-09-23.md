...
- The archive and exported App Store package could not be directly launched on this workstation outside the App Store: local `codesign --verify` returned `CSSMERR_TP_NOT_TRUSTED` and `open` returned `kLSNoExecutableErr`. Xcode's export log nevertheless shows Cloud Managed Apple Distribution signing and App Store Connect reports the binary as verified. This is a local trust/distribution-context failure, not evidence of a Dart startup crash.
- To test the same compiled archive code without changing uploaded bytes, a copy was made under `local-smoke/` and ad-hoc signed only for local diagnosis. That copy launched successfully on macOS 26.5.2; accessibility inspection showed Archive, Document Reading and About, navigation to Document Reading succeeded, and About displayed v1.9.225. The task-owned test process was stopped afterward. This is a local UI smoke pass, **not** a successful installation of the Apple-signed exported package. Public install/launch remains to be verified after approval.
- A release-process hazard was found: the workstation's global Flutter `build-dir` pointed to a separate, running direct-distribution app output. The Store build script was changed after submission to use a task-local Flutter configuration and reject any build output outside the isolated Store cache. A repeat Flutter build completed at `../vibekits-app-store-dev225/flutter-build/.../Vibekits.app` (50.3 MB) with build number 2226 and no external `tools` runtime; the generated Xcode config confirmed the isolated build path. No new binary was uploaded because this script-only fix does not alter the already accepted build; a future archive must use the corrected script.
