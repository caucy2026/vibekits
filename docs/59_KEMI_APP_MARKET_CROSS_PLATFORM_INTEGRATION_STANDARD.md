...
>
> 最后核对：2026-09-10（已对照线上自升级总览、最新接入技能，并补充分页容量、桌面打开和扩展边界）
...
- `os_type` 缺失时，只能在 `platforms` 明确包含兼容平台时展示。当前线上 Android
  目录在 `os=android` 查询下会返回 `android`、`pad2` 或跨平台值 `all`；三者均属于
  Android 客户端的兼容值。macOS/Windows 只接受各自平台值或 `all`，不能把 `pad2`
  泛化到桌面端；
...
- 分类接口中标记 `is_explore` 的“探索”仍是用户可筛选的正式分类，不能因为它是默认探索
  分类就从筛选栏删除。
- 判断“本 APP 已是最新版”必须使用实际安装标识：Android 使用 `applicationId`，macOS/
  Windows 使用稳定商品包名。跨平台商品标识与 Android `applicationId` 不同时必须显式
  归一化，禁止因标识错配重新启用下载按钮。
- 图标失败使用本地占位图，不阻断列表。
...
- 客户端固定使用服务端文档的 `pageSize=30`，从 `page=1` 逐页读取，直到空页、短页或已覆盖合法 `total`；禁止用一次 `pageSize=100` 假设商品永远不增长。
- 合并分页结果时按稳定 `app_id` 去重；缺少有效 ID 时按 `package_name + os_type + version_code` 去重。重复页不能造成 UI 重复、无限请求或内存持续增长。
- 设置有界页数保护。达到保护上限但服务端仍声明有后续数据时，明确返回可重试的格式错误，不能静默截断后宣称列表完整。
- 列表使用懒构建组件；图标和截图按需加载，详情、安装状态与大文件下载不能随目录首屏一并触发。
- 新增平台、商品字段、MCP 提供者或官方 Harness 插件时，均以“未知字段可忽略、已知身份严格校验、列表可分页/虚拟化、单项失败不拖垮全部目录”为扩展原则。
- Harness 的 MCP/官方插件目录不得拼成随数量线性增长的单个 URL。每个应用脚本使用独立受限批次，未来增加插件或 MCP 时只增加批次数，不增加单请求长度；目录 UI 使用懒列表，工具详情按展开项呈现。

### 4.5 同版本禁止重复下载安装

1. 只用稳定 `package_name + 当前 os` 识别本 APP，版本比较只使用整数 `version_code`。
2. 云端版本码等于或小于本机时，卡片、详情和最终下载入口均禁用，不发请求、不启动安装器；未知或非法版本码同样禁用，但不得误称“已是最新版”。
3. 下载入口必须重新读取/核对状态，不能靠旧弹窗或异步刷新前的 UI 状态绕过。
4. 自更新只在服务端业务信封有效、`has_update=true` 且远端整数版本严格更高时提示；失败静默记录，不进入关于页或应用中心。

### 4.6 桌面端打开已安装应用

- 详情操作区提供“打开”，但仅在系统确认对应稳定包名已安装且当前没有下载任务时启用。
- macOS 用 `CFBundleIdentifier` 经 `NSWorkspace` 查询和启动；Windows 读取 `HKCU/HKLM\\Software\\KEMI\\AppMarket\\<package_name>\\Executable` 并验证绝对 `.exe` 路径。
- 禁止用可翻译名称猜路径、把下载历史当安装证据、扫描同名文件或拼接 shell 命令。
- Flutter 与桌面 Runner 统一复用 MethodChannel `org.rustdesk.rustdesk/host` 的 `isStoreApplicationInstalled` 和 `openStoreApplication`，避免各产品分叉协议。
