...
状态：已完成根因定位和macOS 12.6.4本机验证；语法和动态插件隔离已进入 `main`；XHR、`AbortSignal.any`和扩展版本分流已通过自动化门禁，待提交。
...
- `tool/transpile_harness_web_macos.mjs`
  - 对 `dsh-client-connection/lib/client.macos12.js`注入XHR传输；
  - 对 `dsh-api-gateway/lib/client.macos12.js`替换 `AbortSignal.any`；
  - 上游目标代码不存在时立即构建失败，避免DSH升级后静默生成错误产物。
- `test/harness_macos_compatibility_test.dart`
  - 验证官方连接插件仍使用Fetch且不含XHR；
  - 验证兼容连接插件使用XHR；
  - 验证官方Gateway仍保留 `AbortSignal.any`；
  - 验证兼容Gateway不再依赖 `AbortSignal.any`。

不修改DSH业务协议、MCP服务器、模型选择逻辑、会话格式或其他平台启动逻辑。
