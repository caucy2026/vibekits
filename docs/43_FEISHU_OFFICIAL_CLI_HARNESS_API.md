...
Harness 顶栏与“运行日志”“远程分享”同行增加“飞书任务”菜单，提供“看看谁在找我”“汇总今天日程”“整理待办事项”三个只读任务模板。选择后只把完整任务约束填入 Harness 输入框，由用户确认发送。

“谁在找我”存在明确边界：当前官方 CLI 的 IM Schema 没有读取用户全量历史收件箱的接口。只有已配置的飞书消息事件订阅和本地事件归档才能可靠汇总发送人、时间、提及上下文与待回复事项；缺少该链路时 Harness 必须报告缺口，不得编造结果。
