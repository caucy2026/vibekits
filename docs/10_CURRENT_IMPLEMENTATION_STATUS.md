...
当前版本：`1.9.0-dev.47+57`（本机清理联调与工具可见性检查点，非正式发布）
...
| 系统清理 | 五个用户任务入口；Windows 内置规则库 v4（31+ 条）+ 随包外部规则库 v5（首批 7 条）与 macOS v1（26 条）；显式风险/影响；`C:\ESTLOG`、跨用户瞬态发现；全部磁盘容量和逐盘后台分析 | 本机快速扫描 44 秒发现 4.134 GiB 候选；外部规则合并崩溃已修复；全盘 `.log` 已移出默认快速路径；10 GiB永久清理仍等待具体目录授权，不写成完成 |
