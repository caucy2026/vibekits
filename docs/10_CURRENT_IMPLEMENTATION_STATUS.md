...
当前版本：`1.9.0-dev.45+55`（多磁盘占用与安全清理检查点，非正式发布）
...
网络与虚拟化工作区已接入 Mihomo/QEMU 私有运行时解析、独立进程启停、PID/日志状态和 Harness 六项接口。Windows Release 将两者列为强制资产。系统代理/TUN 的旧值恢复闭环、虚拟磁盘创建/快照及 macOS 双架构签名仍未完成，不写成已验收。
...
| 系统清理 | Windows 规则库 v4（31+ 条）与 macOS v1（26 条）；浏览器/应用/系统/开发/IDE/包管理器/日志缓存；`C:\ESTLOG` 超过 24 小时的 `.log` 独立默认规则；跨 `C:\Users\*\AppData\Roaming/Local` 浅层发现；全部可用磁盘即时列出总量/已用/剩余，可多选并逐盘后台分析；系统盘报告区分真实卷容量与含 NTFS 硬链接重复计数的目录逻辑量 | 本机 C/D 两卷原生枚举与双盘交互闭环；数据盘普通目录禁止删除，仅明确缓存/临时/日志可复核清理；`C:\ESTLOG` 24/24 个候选、37.49 MiB 实查闭环；macOS 多卷待实机 |
