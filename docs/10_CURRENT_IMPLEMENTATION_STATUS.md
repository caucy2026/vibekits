...
| 系统清理 | Windows 规则库 v3（31+ 条）与 macOS v1（26 条）；浏览器/应用/系统/开发/IDE/包管理器/日志缓存；`C:\estlog` 旧日志；最多 2 个清理扫描 Isolate；系统盘根项目在独立 Isolate 递归统计，输出总量/已用/剩余、占用来源、合理性、不可读/未归类空间和完整 JSON 报告 | 自动规则、取消、容量/合理性界面与报告主路径完成；Windows 真实全盘性能及 macOS 构建待实机 |
