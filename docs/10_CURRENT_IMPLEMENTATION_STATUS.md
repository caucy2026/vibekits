...
当前版本：`1.9.0-dev.17+27`（Windows 智能清理规则库检查点，非正式发布）
...
| 系统清理 | 浏览器/应用/系统/开发/IDE/插件下载/调试/日志缓存；Windows build 感知规则库、路径/年龄/文件模式约束；最多 2 个扫描 Isolate、低 I/O 占用、扫描/清理取消；本次/累计/系统盘容量总结；白名单、竞态身份、回收站优先、报告 | Windows 规则库 v1 与自动主路径完成；真实并发体验及 macOS 待实机 |
