...
当前版本：`1.9.0-dev.18+28`（开源清理算法吸收检查点，非正式发布）
...
| 系统清理 | 浏览器/应用/系统/开发/IDE/插件下载/调试/日志缓存；Windows build 感知规则库、路径/年龄/最大深度/文件白名单约束；可发现 JetBrains 大型崩溃转储、WSLg ETL、Gradio、.NET、Scoop 等遗漏垃圾；最多 2 个扫描 Isolate、取消、容量总结、白名单、竞态复核、回收站和报告 | Windows 规则库 v2（29 条）与自动主路径完成；真实并发体验及 macOS 待实机 |
