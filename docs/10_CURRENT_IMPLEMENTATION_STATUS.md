...
DeepSeek 从开发工具左侧迁入模型页，界面采用 Codex 风格的工作区、持续对话和底部任务输入。首轮直接发送任务，后续自动携带有界会话上下文；回复支持 Markdown 与复制。运行进度固定在输入框上方，任务期间仍可编辑下一条要求，可随时停止或开始新任务；切换 OCR 后返回不会丢失当前会话。工作区路径与该工作区最近 80 条会话记录跨重启恢复，会话数据保存在用户应用数据目录而不是项目内。`Enter` 发送、`Shift+Enter` 换行。设置页将 DeepSeek 官方 `deepseek-v4-flash`、`deepseek-v4-pro` 作为未验证候选，输入 Key 后以当前端点 `/models` 的真实返回替换列表。输入框权限菜单默认“首次询问”：用户可选择“允许一次”或“本会话允许同类操作”，授权按工具、目标和命令前缀细分；新会话、切换工作区或退出时清除，也可切回“每次询问”。工具日志默认开启，可在 Harness 设置关闭；开发工具页按当前工具过滤展示调用时间、目标、参数摘要、结果、状态和耗时，支持单条删除与清空。ADB 成功/失败证据由 `AdbService` 在真实 `adb.exe` 进程退出后写入原始参数、退出码和 stdout/stderr，桥接层不会伪造成功记录；敏感字段统一脱敏。当前固定官方 `@deepseek-ai/dsh@0.1.0-rc.7 --profile headless`。
