...
| 系统清理 | Windows 规则库 v3（31+ 条）与 macOS v1（26 条）；浏览器/应用/系统/开发/IDE/包管理器/日志缓存；`C:\estlog` 旧日志；跨 `C:\Users\*\AppData\Roaming/Local` 浅层发现有日志证据或明确 cache/log/temp/crash 语义的未知瞬态目录；最多 2 个清理扫描 Isolate；系统盘根项目独立统计并生成完整报告 | 自动规则、取消、容量/合理性和跨用户智能发现主路径完成；未知发现项默认不选择，Windows 真实全盘性能及 macOS 构建待实机 |
