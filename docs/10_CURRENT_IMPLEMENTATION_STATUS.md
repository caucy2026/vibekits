...
当前版本：`1.9.0-dev.46+56`（清理任务入口与外部规则库检查点，非正式发布）
...
| 系统清理 | 五个用户任务入口；Windows 内置规则库 v4（31+ 条）+ 随包外部规则库 v5（首批 7 条）与 macOS v1（26 条）；显式风险/影响；`C:\ESTLOG`、跨用户瞬态发现；全部磁盘容量和逐盘后台分析 | 外部坏规则安全降级、高风险不批量选择和五入口 Widget 已闭环；大文件 MFT/USN 索引、不常用证据、签名在线规则和 macOS 多卷待实现 |
