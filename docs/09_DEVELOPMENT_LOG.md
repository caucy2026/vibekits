...
## 2026-09-24 · Mac dev.236 发布与 4456560334 仿真目录映射

- Mac `1.9.0-dev.236+2236` 已完成 Developer ID 签名、Apple 公证 `Accepted` 和 KEMI 商城现有条目 `53` 更新；本机与远端 `4456560334` 均已运行该版本。
- 远端创建 `/Users/mac/kemi/vibekits`，本机通过仿真通道挂载到 `/Users/newlink/VibeKits-4456560334`。双向创建目录已验证；远端新建项目曾因 rclone 目录缓存延迟不可见，现调整为 2 秒缓存并复验。挂载依赖当前仿真连接，尚未做重启持久化。
- 用户明确本轮只测试远端编译可行性，不迁移源码。误启动的未完成克隆已停止，移除的仅为本轮生成的部分 `.git`；用户目录保留。远端尚缺 Flutter、Cargo 和可用 Xcode，因此没有在该机完成编译。
- 远端 Harness 缓存清理命令已接收但仍在排队，不能记作完成。详见 [本轮改动与验收记录](acceptance/V1_9_0_DEV236_MAC_RELEASE_REMOTE_MAP_2026-09-24.md)。

## 2026-09-21 · dev.224 Harness 派生会话与会话删除收口

- 会话右键菜单新增与官方行高、图标和文字对齐的“派生会话”与“删除会话”；VibeKits 自有操作仅混入少量暖黄色，用于与官方 Harness 操作区分，不使用突兀的独立按钮样式。
- 派生会话先创建真实、持久化的官方空白会话，再在来源与派生会话正文内展示有界整理进度；只在创建时生成一次结构化上下文交接，不复制原聊天时间线，并持久化来源关系与“原名称 2、3……”标题。
- 删除会话采用二次确认并按被右键点击行的真实边界解析目标，避免父容器或滞后选中态导致删错会话；删除后同步核对工作区索引、会话目录和派生关系。
- F1～F12 按当前可见顺序切换会话并恢复输入焦点；普通启动默认进入 Harness；macOS 保存窗口位置、大小和最大化状态；简体中文环境声明及原生菜单本地化随包发布。
- 主程序不主动检查、弹窗、下载或安装自身更新；版本发现与更新只由用户在 KEMI 应用中心主动触发，商城按平台与整数版本号判断所有应用更新。
- 完整 Flutter 回归为 925 项通过、32 项环境门禁跳过、0 失败，静态分析 0 问题。双机发布门禁真实连接 `4456560334` 与 `4567540178` 后分别返回 `HARNESS_WORKSPACE_UNAVAILABLE` 与 `unknown_remote_tool`，因此连续 100 轮与正式 macOS 签名、Apple 公证、KEMI 商城更新仍保持阻断，不把未完成门禁写成发布成功。

## 2026-09-19 · dev.223 远程 Harness 全自动控制与窗口状态记忆

- 远程仿真目标新增 Harness 会话 MCP 闭环：发送命令、读取状态、增量历史、等待变化和停止。控制端连接设备 ID 后可直接让目标电脑自己的 Harness 调用本机工具，正式链路不再依赖截图、OCR 或坐标输入。
- 命令代理要求三秒内返回接受或排队回执，并携带工作区、会话、请求和游标标识；长任务可通过状态变化长轮询持续自动化，等待审批、完成、失败和停止均有结构化状态。
- macOS 主窗口启用系统 frame autosave，退出后再次启动恢复用户最后设置的窗口位置、尺寸和最大化状态。
- 版本更新为 `1.9.0-dev.223+2223`；正式发布证据记录在 `docs/acceptance/V1_9_0_DEV223_REMOTE_HARNESS_AUTOMATION_MARKET_RELEASE_2026-09-19.md`。
...
- 独立中继、仿真目标生命周期和远程控制会话组合测试 29/29 通过；真实目标 `4456560334` 的 P2P 优先与强制 HBBR 两轮都拒绝固定 MCP 端点 `127.0.0.1:32147`，不得据此宣称双机远程仿真完成。
- PAD75 在线且旧版 VibeKits dev.188 与 KEMI 远程办公 PID 独立；从 `c48c0f2` 隔离构建 Android arm64 dev.201 成功，但检测到历史安装包与新 APK 证书不兼容，因此没有卸载清数据。63 当前离线，真机闭环保持 BLOCK。
...
- 停止流程先温和终止，超时后强制终止；进程退出后的流、日志、工具服务器和 Windows Job 释放均设独立有界清理，单个已失效资源不能再永久阻塞任务状态。清理超时只影响该资源的 best-effort 回收，不会把仍活着的 Harness 进程伪报为退出。
- macOS 官方 Harness 的正常任务闭环与真实中途停止现为 `2/2` 通过；相关 Harness、工具服务器、工作区与远程 UI 组合 `50/50` 通过，静态检查 0 issue。Windows 真机必须使用 dev.200 精确源码重复相同两条真实进程测试和 Release 构建后才可继续发布门禁。
...
- PAD 协同模式改为真正的主界面工作区，不再套用弹窗外观；连接前只显示 ID、密码和历史连接，连接后收起表单，仅保留明确状态条、断开入口、远端项目/会话、命令和反馈。
- “已连接”只在 RustDesk P2P/中继、mTLS/hello 和首个有效项目快照都通过后显示；stale/断线立即撤销在线标识，后台自动重连成功后再恢复。
- 远端命令不再把 `{accepted:true}` 当作完成；通过官方 `session/follow` 快照每 2 秒同步权威会话，将流式文本合并为“用户 / Harness / 工具调用 / 本轮完成”，隐藏 system reminder 等内部上下文，原始结构仅在展开详情中保留。
- 75 号 PAD 实测静默连接本机 ID `1554650784`，同步 `harness / READY / 17 会话`，发送命令并收到 `REMOTE_DEV169_FINAL_OK` 与“本轮完成”；执行端重启后 PAD 在无人工点击情况下自动恢复。
- 执行端确认接收命令后清空对应会话输入框；若等待回执期间用户已输入新内容，则按文本与会话双重门禁保留新草稿，避免重复发送或误删输入。
- 75 真机强制 HBBR 中继完成项目同步、命令上行、`HBBR_DEV169_OK` 下行和“本轮完成”；长任务停止返回 `accepted=true`，只产生开头片段即结束并恢复 `READY`。断开历史明确显示“中继 · 1 个工作区”。
- 定向回归 14/14 通过，静态分析 0 issue；最终 PAD APK 为 v2/v3 签名，真机无损覆盖安装和冷启动通过。本版本产品范围为“本机 Mac 执行、PAD 远程协助”，范围内已无剩余功能门禁；Mac↔Mac 若需要应另立需求。本记录不宣称已商店发布。
...
- 首窗不等待网络：启动 3 秒后才低优先级同步；页面永远只展示已经下载并通过 HTTPS、数量、文件名、顺序、大小、MD5、MIME 签名和图片解码验证的本地文件，绝不直接显示远端 URL。
- 使用 `Marketing/kemi_s1_xc` 独立缓存，完整 staging 下载后才原子切换 `active.json`；保留 `backup.json`，下载中断、清单非法或图片损坏时继续显示上一完整版本或随包离线内容。
- 多图每 5 秒自动轮播，点击主图立即下一张，点击指示器直达并重置计时；离开“关于我们”即停止页面计时器，单图不创建计时器。图片数量、资源版本和无障碍语义来自真实清单。
- macOS、Windows、Linux 与移动端继续共用同一个 Flutter 页面和缓存服务；应用版本、LMCP `appVersion` 与 `catalogRevision=2152` 同步。

## 2026-09-03 · 1.9.0-dev.151+2151 · 本地优先“关于我们”

- 按产品化“关于”页规范在“开发工具”之后新增第六个一级入口；macOS、Windows、Linux 与移动端共用同一个 Flutter 页面和导航状态，桌面快捷键同步增加 `Ctrl/Cmd+6`。
- 产品档案只使用 VibeKits 当前真实信息：正式图标、构建版本、本地优先的智能体与工程工具定位，以及 Harness/MCP、压缩、清理、文档和开发工具六类已交付能力；没有复制 KEMI OFFICE 的品牌、格式数量、口号或资源接口。
- VibeKits 当前没有独立宣传资源 HTTPS 接口，因此本版明确采用随包本地内容，不创建无意义轮播、刷新定时器或联网缓存，也不会阻塞启动；关于页在断网时仍完整可读。将来接入独立资源源时必须再实现 HTTPS 校验、active/backup 双指针和原子发布门禁。
- 页面最大宽度 1080px、24px 边距、64px 正式图标、16:9 本地产品区、响应式两列/单列能力卡，并全部复用 VibeKits 明暗主题、颜色、圆角和边框 Token。
- 版本、LMCP `appVersion` 与 `catalogRevision=2151` 同步提升；主界面、小窗口六入口、快捷键、深色主题、版本真实性与 macOS/Windows 共用页面均纳入自动回归。
...
# 2026-08-31 · Windows Release 暴力测试与崩溃证据补强

- 对运行中的 Release 连续执行 600 次 MCP 大目录/真实目录工具调用，0 失败、最大单次 413 ms；随后执行 15 次强制终止、冷启动、窗口响应和 MCP 就绪循环，15/15 成功且没有累积 Node 子进程。
- 在真实 Release 同时运行的资源竞争条件下，主界面、Harness、OCR、设置、拖放、全部开发工具和性能门禁重复 3 轮，共 129 项通过、0 失败；结束工作集 224.8 MiB、745 句柄，没有随轮次持续增长。
...
# 2026-09-01 · dev.141 Harness 输入区对齐与 ADB 管道规范化

- 宽屏 Harness 输入区取消 820px 上限，与上方任务时间线、运行状态条使用同一内容宽度；保留窄窗口响应式布局。
- `vibekits.adb.shell` 识别模型常用的 `dumpsys/getprop | grep` 参数化请求：设备端只执行基础只读命令，`grep -E/-F/-i` 在 VibeKits 进程内过滤。管道和引号不再被 ADB 远程 shell 二次解析，避免先出现一条红色 `no closing quote` 再重试成功。
- LMCP 容量租约的过期计时器改为有租约时才启动，空闲或最后租约释放后立即停止，不再空转或污染 Flutter 生命周期测试。
- 定向回归覆盖 100 指挥官容量竞争、Harness 工具桥、ADB 管道过滤和宽屏 UI：最终 60 项通过，1 项按平台条件跳过；其中正式 App 内置 ADB 对 `192.168.3.63:5555` 的同形只读管道调用为 1/1 通过。
- macOS Release 已发布并运行于项目 `bin/Vibekits.app`，版本 `1.9.0-dev.141+2141`；深度签名校验通过，UDP `*:47831` 监听正常。正式 UI 截图确认输入区与会话区对齐；旧 dev.140 App 保存在 `build/previous/Vibekits-dev140-before-dev141.app`。

# 2026-09-01 · dev.142 Harness 多会话独立并行与选中态操作

- Harness 任务运行状态从全局单例改为 `workspace + sessionId` 独立运行上下文；每个会话拥有自己的模型进程句柄、输出订阅、停止/清理状态、执行时间线和消息缓存。
- 一个会话正在推理时，切换到另一个会话后输入框、发送和停止控制只绑定当前会话；可以继续输入并启动第二个并行任务，两边输出不串会话。
- 项目和会话列表统一为 Codex 式选中语义：只有选中项显示 `…`；未选中但运行中显示转圈；未选中且空闲时右侧留空。项目右键菜单仍可用。
- 多会话并行保存对同一工作区串行化，避免多个任务同时替换同一临时文件。Harness 全局 BUSY 只在最后一个任务结束时恢复空闲。
- 定向 Widget 回归覆盖跨项目并行、同项目双会话并行、草稿隔离、选中菜单、未选中转圈和最后任务结束才发布全局空闲。
- macOS Release 已发布并运行于 `bin/Vibekits.app`，版本 `1.9.0-dev.142+2142`；20 个 Mach-O ad-hoc 重签后通过启动后深度验签，UDP `*:47831` 监听正常。旧 dev.141 保存在 `build/previous/Vibekits-dev141-before-dev142.app`。

# 2026-09-01 · dev.143 Codex 式执行时间线与 Intel Mac 发布门禁

- 运行中和历史执行时间线统一默认折叠，顶部只显示完成步数和当前动作；展开后按“理解→规划→工具→继续分析→结果”阅读，不再把整块 JSON 直接铺在主会话。
...
## 2026-09-13 · 1.9.0-dev.202+2202 · 远程仿真一次授权持续复用

- 被控端打开“远程仿真”开关视为对该功能范围的一次持续授权；同一控制端 VibeKits ID 通过原生 RustDesk 仿真通道认证后，安装、卸载和受管 SSH 公钥操作不再逐次弹出应用内批准。
...
- 目标 `4456560334` 安装后真实连接与 204 项目录成功，但首个只读工具调用超时；dev.202 随即撤回。根因是授权身份查询错误覆盖所有工具调用，而不是只覆盖四个敏感工具。dev.203 将查询收窄到安装、卸载、SSH 授权和撤销，并新增“只读工具绝不触发授权身份查询”的永久回归。

## 2026-09-13 · 1.9.0-dev.203+2203 · 只读调用解耦与完整应用生命周期门禁

- 修复 dev.202 真实机暴露的授权查询耦合：只读 MCP 工具不再访问原生敏感授权身份查询；安装、卸载、SSH 公钥授权与撤销仍要求同一条已认证原生连接。
- 全量 Flutter 回归 865 项通过、21 项显式环境门禁跳过、0 失败；远程协助/仿真/证书/mTLS/项目会话/命令反馈停止/P2P-HBBR/macOS-Windows 共用逻辑专项 170 项通过、4 项真实环境门禁跳过。
...
- Windows 58 dev.205 Release 已完成 41 项自包含运行时验证与真实启动，便携 ZIP 为 319,553,505 bytes，SHA-256 `929e05ed4b294f2cdbd72e7a5d98010e615d94889d34bc2cb2b0bb2200792b38`。
...
## 2026-09-16 · Harness 运行中纠正与可读性升级

- 官方 Harness 固定版本由 `@deepseek-ai/dsh@0.1.2-rc.1` 升到 `0.1.5-rc.2`，macOS 与 Windows 共用同一补丁和交互资产；保留上游队列能力，但新配置默认将运行中输入作为同一轮“补充并纠正”。
- 新输入在最近的安全步骤边界进入当前推理；同一会话保留全部输入和已验证结果，前后冲突时以后输入为准。中文按钮和设置名称同步改为“补充并纠正”。
- 推理正文恢复 14px/22px 的主阅读层级，过程摘要为 13px/20px，原始工具输出和长时间线继续默认折叠；外部待执行数量为 0 时不再显示无意义控件。
- macOS 12+ Universal 隔离运行时从零生成成功，CLI 报告 `0.1.5-rc.2`，Node 为 `x86_64 + arm64`，跨项目会话重绑定通过；Harness 会话、队列、共享 UI 与服务定向回归 `46/46`，全量静态分析 0 issue。
- 本条仅记录源码和隔离运行时门禁，不冒充完整 App 签名、公证或正式发布。
# 2026-09-16 · dev.216 远程仿真按需传输死锁修复

- 修复控制器先等待隧道、后发送本地请求造成的 45 秒确定性超时；控制、MCP、移动端复用通道均改为先发送真实请求触发原生按需转发，再等待连接完成。
...
# 2026-09-19：VibeKits 更新入口收敛到应用中心并默认进入 Harness

- 修正旧构建恢复后再次出现全局升级弹窗的问题。VibeKits 主程序不再拥有主动自更新能力，启动、后台及兼容 API 调用均不得检查、提示、下载或安装自身更新。
- VibeKits 的版本发现和更新统一由“应用中心”商城页面承载，并且只在用户主动操作后执行。
- 普通启动固定进入“智能体（Harness）”，不再被上次关闭时停留的一级页面覆盖；文件打开和明确工作区入口仍按目标路由。
- 新增 macOS/Windows 更新边界回归测试，并修改启动页面测试，验证兼容更新服务零网络请求且默认页为 Harness。
- F1–F12 会话切换改为点击会话条目内真实可交互元素，滚动选中项到可见区域，并在路由完成后再次聚焦输入框，修复 F2 切换后选中态和输入焦点没有恢复的问题。
- macOS 包新增简体中文本地化声明和菜单资源；中文系统中的 WKWebView 原生右键菜单随系统显示中文，英文系统继续使用英文。
- Harness 会话右键菜单新增“新开会话”，调用官方“新建会话”入口创建空白会话，不复制原聊天记录；创建后恢复输入框焦点。
- 2026-09-19：依据随包官方 `@deepseek-ai/dsh-client-ui-settings-unarchive-sessions` 源码与真机界面核验，归档会话的官方查询入口为“设置 → 已归档会话”，页面提供搜索与“取消归档”，不提供永久删除。删除了 VibeKits 曾错误注入到“视图选项”的重复归档入口；保留官方“分叉会话”“归档会话”。新增的压缩上下文继续开发入口统一显示为“新会话”，悬停说明其行为。

# 2026-09-20：dev.223 远程 Harness 命令与派生会话正式发布

- 发布远程 Harness 命令通道：控制端可按仿真设备 ID 提交任务，并读取状态、增量历史、等待变化或取消；目标机仍由自己的官方 Harness 展示聊天与执行本机工具。
- “派生会话”只在创建时生成一次语义交接摘要，创建真实持久化新会话，不复制原聊天记录；进度状态只显示在来源与派生会话内。
- macOS 正式候选完成精确路径实启、Harness tool bridge、Developer ID、Apple 公证、staple、Gatekeeper、Universal 和 CDN 回下载复验。Apple Submission ID `7cf1b828-c163-4f33-a961-41b3b398d1b5`；商城包 `420443201` bytes，SHA-256 `e1d921b218e1aa15076331ac7334b360c04469fbfd043d87c50a45b4cc00a50b`。
- KEMI 商场 `app_id=53` 已更新到 `1.9.0-dev.223 / 2223`；旧版 `2222` 命中更新，当前版 `2223` 不重复提示。更新仅从应用中心由用户主动执行。
- 修复签名脚本可能按 Bundle ID 启动错误候选的问题，并在验收结束后按精确路径清理 Harness Relay；最终复查无 Relay 残留，避免旧测试进程持续占用 CPU。
