...
- 建立 `docs/63_MAC_APP_STORE_DISTRIBUTION_COMPLIANCE_PLAN.md`：根据 Apple 现行审核规则，明确禁止“使用三次后显示”这类审核规避；规划 Direct 全功能版与永久移除站外安装/更新、启用 App Sandbox 的 Mac App Store 变体。当前机器实查为 0 个有效 codesigning identity、0 个 provisioning profile，现有 Release entitlements 也未启用 App Sandbox，不将 Developer ID 站外证书冒充为 Mac App Store 发行条件。
