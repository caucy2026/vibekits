...
## 2026-09-08 · 1.9.0-dev.162+2162 · 多语言与桌面后台常驻
...
- macOS 点击关闭窗口仅隐藏，Harness、MCP 发现及长任务继续运行；点击 Dock 可恢复窗口，显式“退出 VibeKits”才结束进程。Windows 沿用同一语义：关闭到托盘，托盘菜单显式退出。
...
- 2026-09-08 使用既有开发者账号进入 Uptodown 控制台复查：VibeKits（后台应用 ID `1000852511`）状态为“已拒绝”，通知时间为 2026-08-09 14:30，平台原文为“不符合在 Uptodown 上发布的最低质量标准”；详情页同时显示 `DRAFT`，截图、描述、文件计数均为 0，图标也未提交。KEMI 发送状态为“待修订”。因此此前“未提交”的结论已被后台新证据纠正；当前也不能写成“审核中”或“已发布”。本轮按真实缺失项准备图标、截图、完整描述与 Developer ID 公证安装包，不把 KEMI 应用中心臆断为拒绝原因。
- 建立 `docs/63_MAC_APP_STORE_DISTRIBUTION_COMPLIANCE_PLAN.md`：根据 Apple 现行审核规则，明确禁止“使用三次后显示”这类审核规避；规划 Direct 全功能版与永久移除站外安装/更新、启用 App Sandbox 的 Mac App Store 变体。受限沙箱曾误报 0 个 identity；正常 macOS 安全上下文实查可用 Developer ID 和 `KEMI_NOTARY`，但没有 Mac App Distribution/Mac Installer Distribution 证书和 Mac App Store provisioning profile，现有 Release entitlements 也未启用 App Sandbox。
- dev.162 Direct macOS 候选已用正式 Developer ID 签名并获得 Apple 公证 `Accepted`（Submission ID `f8d1fc8f-b149-452f-9009-9be7a3a4d456`）；staple、Gatekeeper、35 个 Mach-O、Harness/Node/JIT/DSH、Universal x86_64+arm64 和 macOS 12.0 门禁通过。公证后 ZIP 为 290,680,125 bytes，SHA-256 `d3522b62354dfaa6da2f28918313fa39f51a7fb64d062e752cf1f1239d606820`。该包是 Direct/Uptodown 候选，不冒充 Mac App Store 包。
