...
- Windows 当前固定产物：`@deepseek-ai/dsh@0.1.2-rc.1`
...
| 检查项 | `@deepseek-ai/dsh@0.1.2-rc.1` 官方行为 | Vibekits 当前处理 |
|---|---|---|
...
| 会话独立输入草稿 | 每个 `sessionId` 独立保存文本草稿；A 输入 `111`、B 输入 `222`，往返切换分别恢复 `111`/`222` | 草稿恢复必须监听会话 store 的异步水合与切换，发送成功只清当前会话；不得用全局 composer 草稿串写其他会话 |
