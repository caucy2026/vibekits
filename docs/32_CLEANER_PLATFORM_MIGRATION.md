...
8. Gradle 缓存不逐文件拆散清理：构建缓存按 24 小时未更新的完整顶层目录聚合，`.gradle/.tmp` 只列 1 小时前文件。路径和可重建性证据明确，因此允许在最终确认后作为“强力清理可再生缓存”永久删除，避免只移入回收站而不释放物理空间；`devCache` 高风险分类仍保证其不会零确认自动执行，文件身份变化或进程占用时跳过。这使满盘机器能看到并真正释放近期大容量缓存，仍不触碰项目源码和 Gradle 配置。
9. Windows `Users` 目录新增“其他用户配置容量盘点”深度规则：排除当前用户、Public/Default 等系统模板，对其他配置只做目录级容量汇总。候选固定标记为系统管理/受保护，不进入自动恢复计划；原因是目录可能含个人文档、SSH 密钥以及已加载的 `NTUSER.DAT` / `UsrClass.dat`，普通文件删除器无法安全判定账户已废弃，也不得静默取得所有权或卸载注册表配置单元。
