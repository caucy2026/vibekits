...
| `dartssh2` | 2.22.5 | SSH 密码/私钥认证、主机指纹、远程 PTY 与 SFTP | MIT | pub.dev 锁文件；固定在 `pubspec.lock`，Vibekits 强制提供主机密钥验证回调且不关闭验证 |
...
- SSH 交互终端与 SFTP 使用 `dartssh2`，终端渲染使用 `xterm`；主机密钥必须经用户确认或与已绑定指纹一致。端口转发仍待 R9 后续实现；不自行实现密码学。
