...
import 'github_cli_service.dart';
...
typedef HarnessMcpSchedulePlanner =
    Future<Map<String, Object?>> Function(String toolName, String taskId);
typedef HarnessMcpAutoInvoker =
    Future<Map<String, Object?>> Function(
      String toolName,
      String taskId,
...
typedef HarnessAgentActivityProbe = bool Function();
...
class VibekitsHarnessToolBridge {
  factory VibekitsHarnessToolBridge({
    ...
    HarnessMcpCatalogLoader? mcpCatalogLoader,
    HarnessMcpToolInvoker? mcpToolInvoker,
    HarnessMcpSchedulePlanner? mcpSchedulePlanner,
  ...
  VibekitsHarnessToolBridge._(
    ...
    this._mcpCatalogLoader,
    this._mcpToolInvoker,
    this._mcpSchedulePlanner,
  ...
  static const String mcpCatalogListId = 'vibekits.mcp.catalog_list';
  static const String mcpToolCallId = 'vibekits.mcp.tool_call';
  static const String mcpSchedulePlanId = 'vibekits.mcp.schedule_plan';
  static const String mcpAutoCallId = 'vibekits.mcp.auto_call';
  static const String mcpReputationListId = 'vibekits.mcp.reputation_list';
  static const String mcpReputationRateId = 'vibekits.mcp.reputation_rate';
  static const String githubDiagnosticsId = 'vibekits.github.diagnose';
  static const String githubCliInspectId = 'vibekits.github.cli_inspect';
  static const String githubCliAuthStatusId = 'vibekits.github.cli_auth_status';
  static const String githubCliExecuteId = 'vibekits.github.cli_execute';
  ...
  static const String agentCliCatalogId = 'vibekits.agent_cli.catalog';
  static const String agentCliInspectId = 'vibekits.agent_cli.inspect';
  static const String agentCliExecuteId = 'vibekits.agent_cli.execute';
  static const String agentCliTaskStartId = 'vibekits.agent_cli.task_start';
  static const String agentCliTaskStatusId = 'vibekits.agent_cli.task_status';
  static const String agentCliTaskCancelId = 'vibekits.agent_cli.task_cancel';
  ...
  final HarnessMcpCatalogLoader? _mcpCatalogLoader;
  final HarnessMcpToolInvoker? _mcpToolInvoker;
  final HarnessMcpSchedulePlanner? _mcpSchedulePlanner;
  ...
  late final GithubCliService _githubCliService = GithubCliService(
    executable: _runtimeToolRoot == null
        ? null
        : '$_runtimeToolRoot${Platform.pathSeparator}github-cli'
              '${Platform.pathSeparator}${Platform.isWindows ? 'gh.exe' : 'gh'}',
  );
  late final AgentCliService _agentCliService = AgentCliService(
    runtimeToolRoot: _runtimeToolRoot,
  ...
  _definitions = <String, HarnessToolDefinition>{
    ...
    mcpToolCallId: _definition(
      ...
      description:
          '按最新目录把调用路由到指定的本地其他进程或经过认证的局域网 MCP 实例。必须先遵循 app → local → lan 的固定层级，只在同层按全局工具评分降序选择；原样使用 catalog_list 返回的 instanceId、toolName 和 inputSchema。调用方服从当前 Harness 权限模式；被调用 APP 首次建立工具/风险/资源授权，后续命中持久范围时自动执行并显示可强制关闭的运行提示，不得逐次重复审批。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'instanceId': _string('catalog_list 返回的目标 MCP 实例 ID'),
        'toolName': _string('目标 tools/list 中的真实工具名'),
        'arguments': const <String, Object?>{
          'type': 'object',
          'description': '严格符合目标工具 inputSchema 的参数对象',
          'additionalProperties': true,
        },
      },
      required: const <String>['instanceId', 'toolName', 'arguments'],
    ),
    mcpSchedulePlanId: _definition(
      id: mcpSchedulePlanId,
      name: '规划 MCP 作战单位调度',
      description:
          '按 LMCP/2 指挥官标准过滤当前在线、可调用、有实时空闲槽并实现租约控制的同类 MCP 节点；保持 app→local→lan 层级，同层按容量、全局质量、可靠性、延迟、新鲜度和稳定打散评分。本工具只生成可审计计划，不占用容量也不执行业务工具。',
      properties: <String, Object?>{
        'toolName': _string('tools/list 中的真实业务工具名'),
        'taskId': _string('全局稳定任务 ID，用于同分节点稳定打散'),
      },
      required: const <String>['toolName', 'taskId'],
    ),
    mcpAutoCallId: _definition(
      id: mcpAutoCallId,
      name: '自动调度并调用 MCP 作战单位',
      description:
          '读取实时目录，在最高优先层选择有空闲容量且评分最优的同类节点，先原子预约，再执行真实业务工具，最后释放租约；节点忙碌会自动尝试下一候选。返回选择理由、脱敏租约 ID、完整业务结果和失败尝试，不返回 leaseToken。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'toolName': _string('要完成任务的真实业务工具名'),
        'taskId': _string('本次工程任务的稳定 ID'),
        'idempotencyKey': _string('重试保持不变的幂等键'),
        'scopeDigest': _string('授权资源范围摘要'),
        'arguments': const <String, Object?>{
          'type': 'object',
          'additionalProperties': true,
        },
    ...
    mcpReputationRateId: _definition(
      ...
      properties: <String, Object?>{
        'tier': <String, Object?>{
          'type': 'string',
          'enum': <String>['app', 'local', 'lan'],
          'description': '当前目录层级，用于审计；评分本身按 toolName 全局共享',
        },
        'instanceId': _string('当前目录中的实例 ID，用于审计，不作为评分主键'),
        'toolName': _string('tools/list 中的真实工具名；同名工具跨设备共享评分'),
        'rating': <String, Object?>{
          'type': 'integer',
          'minimum': 0,
          'maximum': 5,
          'description': '0=垃圾，1=很差，2=较差，3=一般，4=好用，5=非常好用',
        },
      },
      required: const <String>['tier', 'instanceId', 'toolName', 'rating'],
    ),
    ...
    adbShellId: _definition(
      ...
      description:
          '对选定设备执行参数化 Android shell 命令；不经过本机 cmd 或 sh。读取多个系统属性时可把多个合法属性名放在同一次 getprop 调用中，VibeKits 会安全拆分并返回属性映射。对 dumpsys/getprop 后的 `| grep [-E|-F|-i] pattern` 会在本地安全过滤，不把管道或引号交给设备 shell 重新解析。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'serial': _string('设备序列号或 IP:端口'),
        'arguments': <String, Object?>{
    ...
    lanPeersListId: _definition(
      ...
      name: '发现局域网MCP协同节点',
      description:
          '按 LMCP/2 列出同一私网内 VibeKits 或第三方 APP 主动广播的 HTTPS MCP 节点；仅通过 TLS 指纹、双端点和目录摘要校验的节点可调用。旧 LMCP/1 ssh-stdio 节点只显示“仅发现、不可调用”；第三方实现必须遵循 docs/50_LMCP_APP_DEVICE_IDENTITY_AND_SWITCH_STANDARD.md。',
      properties: const <String, Object?>{},
    ),
    ...
    githubCliInspectId: _definition(
      id: githubCliInspectId,
      name: '检查内置 GitHub CLI',
      description: '只读返回 APP 内置官方 gh 的版本、平台和运行时路径；不依赖系统 PATH。',
      properties: const <String, Object?>{},
    ),
    githubCliAuthStatusId: _definition(
      id: githubCliAuthStatusId,
      name: '检查 GitHub CLI 授权',
      description: '非交互执行 gh auth status，可检查 GitHub.com 或企业主机；不会返回认证令牌。',
      properties: <String, Object?>{
        'hostname': _string('可选 GitHub 主机名，例如 github.com'),
      },
    ),
    githubCliExecuteId: _definition(
      id: githubCliExecuteId,
      name: '执行 GitHub CLI',
      description:
          '以参数数组直接调用 APP 内置官方 gh，不经过 shell。支持 repo、pr、issue、run、release、api 等命令；关闭交互提示，禁止通过参数传入或读取令牌，远程调用需授权。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'arguments': const <String, Object?>{
          'type': 'array',
          'description':
              'gh 后的逐项参数，例如 ["pr","list","--repo","owner/repo","--json","number,title"]，不得拼成 shell 字符串',
          'items': <String, Object?>{'type': 'string'},
          'minItems': 1,
          'maxItems': 128,
        },
        'workingDirectory': _string('可选本地仓库或工作目录绝对路径'),
        'timeoutSeconds': const <String, Object?>{
          'type': 'integer',
          'minimum': 5,
          'maximum': 3600,
    ...
    agentCliCatalogId: _definition(
      id: agentCliCatalogId,
      name: '列出智能体 CLI',
      description:
          '实时发现 Codex、Claude Code、GitHub Copilot、Cursor Agent、Gemini、Aider 和 OpenCode；返回真实路径、来源、版本与协议能力，不触发登录或任务。',
      properties: const <String, Object?>{},
    ),
    agentCliInspectId: _definition(
      id: agentCliInspectId,
      name: '检查智能体 CLI',
      description: '重新解析一个智能体 CLI 并执行有界版本探测；未安装时明确返回 unavailable，不自动替换提供方。',
      properties: <String, Object?>{'providerId': _agentCliProviderIdSchema},
      required: <String>['providerId'],
    ),
    agentCliExecuteId: _definition(
      id: agentCliExecuteId,
      name: '执行智能体 CLI',
      description:
          '以参数数组同步调用指定智能体 CLI，不经过 shell、不接受 stdin、不允许参数携带密钥；适合短任务，远程调用必须审批。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'providerId': _agentCliProviderIdSchema,
        'arguments': _agentCliArgumentsSchema,
        'workingDirectory': _string('可选且必须存在的工作目录绝对路径'),
        'timeoutSeconds': const <String, Object?>{
          'type': 'integer',
          'minimum': 5,
          'maximum': 3600,
      ...
      required: <String>['providerId', 'arguments'],
    ),
    agentCliTaskStartId: _definition(
      id: agentCliTaskStartId,
      name: '启动智能体 CLI 长任务',
      description: '启动后立即返回 taskId；后台并行读取 stdout/stderr，并由 VibeKits 管理超时和进程树。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{
        'providerId': _agentCliProviderIdSchema,
        'arguments': _agentCliArgumentsSchema,
    ...
    agentCliTaskStatusId: _definition(
      id: agentCliTaskStatusId,
      name: '查询智能体 CLI 长任务',
      description: '按 taskId 返回运行状态、PID、退出码、有界输出和截断证据；终态保留30分钟。',
      properties: <String, Object?>{'taskId': _string('长任务 ID')},
      required: <String>['taskId'],
    ),
    agentCliTaskCancelId: _definition(
      id: agentCliTaskCancelId,
      name: '取消智能体 CLI 长任务',
      description: '只取消当前 VibeKits 实例创建的任务；重复取消幂等，不能按外部 PID 杀进程。',
      risk: HarnessToolRisk.controlsDevice,
      properties: <String, Object?>{'taskId': _string('长任务 ID')},
      required: <String>['taskId'],
    ),
  ...
  Future<HarnessToolCallResult> invoke({
    ...
    if ((Platform.isAndroid || Platform.isIOS) && requiresDesktopNode(toolId)) {
      ...
      _publishToolSettledStatus(
        directPhase: HarnessWorkPhase.ready,
        directMessage: '${definition.name}需要桌面节点',
        orchestratedMessage: '${definition.name}需要桌面节点，Harness 继续分析',
        toolId: toolId,
        toolName: definition.name,
        target: target,
      );
    ...
    if (definition.risk != HarnessToolRisk.readOnly && !preauthorized) {
      ...
      if (!allowed) {
        ...
        _publishToolSettledStatus(
          directPhase: HarnessWorkPhase.ready,
          directMessage: '已拒绝 ${definition.name}',
          orchestratedMessage: '已拒绝 ${definition.name}，Harness 继续分析',
          toolId: toolId,
          toolName: definition.name,
          target: target,
        );
    ...
    if (!_agentOrchestrated || _agentIsActive) {
      HarnessWorkStatusHub.publish(
    ...
    try {
      ...
      _publishToolSettledStatus(
        directPhase: HarnessWorkPhase.ready,
        directMessage: '${definition.name} 执行成功',
        orchestratedMessage: '${definition.name} 执行成功，Harness 继续分析',
        toolId: toolId,
        toolName: definition.name,
        target: target,
      );
      ...
      _publishToolSettledStatus(
        directPhase: HarnessWorkPhase.failed,
        directMessage: '${definition.name} 执行失败',
        orchestratedMessage: '${definition.name} 执行失败，Harness 正在调整方案',
        toolId: toolId,
        toolName: definition.name,
        target: target,
      );
  ...
  bool get _agentIsActive => _agentActive?.call() ?? true;

  void _publishToolSettledStatus({
    required HarnessWorkPhase directPhase,
    required String directMessage,
    required String orchestratedMessage,
    required String toolId,
    required String toolName,
    required String target,
  }) {
    if (_agentOrchestrated && !_agentIsActive) return;
    HarnessWorkStatusHub.publish(
      phase: _agentOrchestrated ? HarnessWorkPhase.reasoning : directPhase,
      message: _agentOrchestrated ? orchestratedMessage : directMessage,
      toolId: toolId,
      toolName: toolName,
      target: target,
    );
  ...
  HarnessToolHandler? _handlerFor(String toolId) {
    ...
    if (toolId == mcpCatalogListId) return _listMcpCatalog;
    if (toolId == mcpToolCallId) return _callMcpTool;
    if (toolId == mcpSchedulePlanId) return _planMcpSchedule;
    if (toolId == mcpAutoCallId) return _autoCallMcp;
    if (toolId == mcpReputationListId) return _listMcpReputations;
    if (toolId == mcpReputationRateId) return _rateMcpTool;
    if (toolId == githubDiagnosticsId) return _diagnoseGithub;
    if (toolId == githubCliInspectId) return _inspectGithubCli;
    if (toolId == githubCliAuthStatusId) return _githubCliAuthStatus;
    if (toolId == githubCliExecuteId) return _executeGithubCli;
    ...
    if (toolId == agentCliCatalogId) return _agentCliCatalog;
    if (toolId == agentCliInspectId) return _agentCliInspect;
    if (toolId == agentCliExecuteId) return _agentCliExecute;
    if (toolId == agentCliTaskStartId) return _agentCliTaskStart;
    if (toolId == agentCliTaskStatusId) return _agentCliTaskStatus;
    if (toolId == agentCliTaskCancelId) return _agentCliTaskCancel;
  ...
  Future<Map<String, Object?>> _runAdbShell(
    ...
    final String? launchedPackage = _launchedAndroidPackage(values);
    if (_agentOrchestrated && launchedPackage != null) {
      final String serial = (arguments['serial'] ?? '').toString().trim();
      _agentOwnedAndroidPackages['$serial\u0000$launchedPackage'] =
          launchedPackage;
      result['agentOwnedActivity'] = <String, Object?>{
        'serial': serial,
        'package': launchedPackage,
        'cleanup': 'am force-stop',
      };
    }
    return result;
  }

  static String? _launchedAndroidPackage(List<String> values) {
    if (values.length < 3 ||
        values[0].toLowerCase() != 'am' ||
        values[1].toLowerCase() != 'start') {
      return null;
    }
    final int componentIndex = values.indexOf('-n');
    if (componentIndex < 0 || componentIndex + 1 >= values.length) return null;
    final String component = values[componentIndex + 1];
    final String package = component.split('/').first.trim();
    return RegExp(r'^[A-Za-z][A-Za-z0-9_.]{2,127}$').hasMatch(package)
        ? package
        : null;
  }

  Future<List<Map<String, Object?>>> stopAgentOwnedActivities() async {
    final List<Map<String, Object?>> cleaned = <Map<String, Object?>>[];
    final List<String> owned = _agentOwnedAndroidPackages.keys.toList();
    for (final String identity in owned) {
      final int separator = identity.indexOf('\u0000');
      if (separator <= 0) continue;
      final String serial = identity.substring(0, separator);
      final String package = identity.substring(separator + 1);
      try {
        await _executeSemanticAdb(
          <String, Object?>{'serial': serial},
          <String>['shell', 'am', 'force-stop', package],
        );
        final AdbCommandResult probe = await _runSemanticAdbRaw(
          serial,
          <String>['shell', 'pidof', package],
        );
        final bool verifiedStopped =
            probe.exitCode != 0 || probe.stdout.trim().isEmpty;
        cleaned.add(<String, Object?>{
          'serial': serial,
          'package': package,
          'stopped': verifiedStopped,
          'verified': verifiedStopped,
          if (!verifiedStopped) 'remainingPids': probe.stdout.trim(),
        });
        if (verifiedStopped) _agentOwnedAndroidPackages.remove(identity);
      } on Object catch (error) {
        cleaned.add(<String, Object?>{
          'serial': serial,
          'package': package,
  ...
  Future<AdbCommandResult> _runSemanticAdbRaw(
    String serial,
    List<String> command,
  ) {
    final String executable =
        _adbExecutable ?? AdbService.bundledExecutablePath();
    final List<String> adbArguments = <String>['-s', serial, ...command];
    return _adbRunner == null
        ? AdbService.runCommand(
            executable,
            adbArguments,
            endpoint: _activeAdbEndpoint,
            timeout: const Duration(seconds: 30),
          )
        : _adbRunner(executable, _activeAdbEndpoint.applyTo(adbArguments));
  }
  ...
  Future<Map<String, Object?>> _inspectGithubCli(
    Map<String, Object?> arguments,
  ) => _githubCliService.inspect();

  Future<Map<String, Object?>> _githubCliAuthStatus(
    Map<String, Object?> arguments,
  ) => _githubCliService.authStatus(
    hostname: (arguments['hostname'] ?? '').toString(),
  );

  Future<Map<String, Object?>> _executeGithubCli(
    Map<String, Object?> arguments,
  ) {
    final Object? rawArguments = arguments['arguments'];
    if (rawArguments is! List) {
    ...
    final List<String> cliArguments = rawArguments
        ...
        .toList(growable: false);
    final int timeoutSeconds =
        (arguments['timeoutSeconds'] as num?)?.toInt().clamp(5, 3600) ?? 300;
    return _githubCliService.execute(
      cliArguments,
      workingDirectory: (arguments['workingDirectory'] ?? '').toString(),
      timeout: Duration(seconds: timeoutSeconds),
    );
  }
  ...
  Future<Map<String, Object?>> _agentCliCatalog(
    Map<String, Object?> arguments,
  ) => _agentCliService.catalog();

  Future<Map<String, Object?>> _agentCliInspect(
    Map<String, Object?> arguments,
  ) => _agentCliService.inspect((arguments['providerId'] ?? '').toString());

  Future<Map<String, Object?>> _agentCliExecute(
    Map<String, Object?> arguments,
  ) => _agentCliService.execute(
    (arguments['providerId'] ?? '').toString(),
    _agentCliArguments(arguments),
    workingDirectory: (arguments['workingDirectory'] ?? '').toString(),
    timeout: Duration(
      seconds:
          (arguments['timeoutSeconds'] as num?)?.toInt().clamp(5, 3600) ?? 300,
  ...
  Future<Map<String, Object?>> _agentCliTaskStart(
    Map<String, Object?> arguments,
  ) => _agentCliService.startTask(
    (arguments['providerId'] ?? '').toString(),
    _agentCliArguments(arguments),
    workingDirectory: (arguments['workingDirectory'] ?? '').toString(),
    timeout: Duration(
      seconds:
          (arguments['timeoutSeconds'] as num?)?.toInt().clamp(5, 86400) ??
          3600,
    ),
  );

  Future<Map<String, Object?>> _agentCliTaskStatus(
    Map<String, Object?> arguments,
  ) async => _agentCliService.status((arguments['taskId'] ?? '').toString());

  Future<Map<String, Object?>> _agentCliTaskCancel(
    Map<String, Object?> arguments,
  ) => _agentCliService.cancel((arguments['taskId'] ?? '').toString());

  static List<String> _agentCliArguments(Map<String, Object?> arguments) {
    final Object? raw = arguments['arguments'];
    if (raw is! List) throw const FormatException('arguments 必须是字符串数组');
    return raw
        .map((Object? value) => value is String ? value : '')
        .toList(growable: false);
  }
  ...
  Future<Map<String, Object?>> _listMcpCatalog(
    Map<String, Object?> arguments,
  ) async {
    final HarnessMcpCatalogLoader? loader = _mcpCatalogLoader;
    if (loader == null) {
      throw StateError('当前 Harness 尚未连接 MCP 能力目录');
    }
    return loader();
  }
  ...
  Future<Map<String, Object?>> _callMcpTool(
    ...
    final HarnessMcpToolInvoker? invoker = _mcpToolInvoker;
    if (invoker == null) {
      throw StateError('当前 Harness 尚未连接 MCP 调用路由器');
    }
    final String instanceId = '${arguments['instanceId'] ?? ''}'.trim();
    final String toolName = '${arguments['toolName'] ?? ''}'.trim();
    final Object? rawArguments = arguments['arguments'];
    if (instanceId.isEmpty || toolName.isEmpty || rawArguments is! Map) {
      throw const FormatException('instanceId、toolName 和 arguments 对象均为必填项');
    }
    return invoker(instanceId, toolName, rawArguments.cast<String, Object?>());
  }

  Future<Map<String, Object?>> _planMcpSchedule(
    Map<String, Object?> arguments,
  ) async {
    final HarnessMcpSchedulePlanner? planner = _mcpSchedulePlanner;
    if (planner == null) {
      throw StateError('当前 Harness 尚未连接 MCP 指挥官调度器');
    }
    final String toolName = '${arguments['toolName'] ?? ''}'.trim();
    final String taskId = '${arguments['taskId'] ?? ''}'.trim();
    if (toolName.isEmpty || taskId.isEmpty) {
      throw const FormatException('toolName 和 taskId 均为必填项');
    }
    return planner(toolName, taskId);
  }

  Future<Map<String, Object?>> _autoCallMcp(
    Map<String, Object?> arguments,
  ) async {
    final HarnessMcpAutoInvoker? invoker = _mcpAutoInvoker;
    if (invoker == null) {
      throw StateError('当前 Harness 尚未连接 MCP 自动调度执行器');
    }
    final String toolName = '${arguments['toolName'] ?? ''}'.trim();
    final String taskId = '${arguments['taskId'] ?? ''}'.trim();
    final String idempotencyKey = '${arguments['idempotencyKey'] ?? ''}'.trim();
    final String scopeDigest = '${arguments['scopeDigest'] ?? ''}'.trim();
    final Object? rawToolArguments = arguments['arguments'];
    if (toolName.isEmpty ||
        taskId.isEmpty ||
        idempotencyKey.isEmpty ||
        scopeDigest.isEmpty ||
        rawToolArguments is! Map) {
      throw const FormatException('自动调度工具参数不完整');
    }
    return invoker(
      toolName,
      taskId,
      idempotencyKey,
      scopeDigest,
      rawToolArguments.cast<String, Object?>(),
      _integer(arguments['requestedSlots'], 1),
      _integer(arguments['ttlSeconds'], 45),
    );
  }

  Future<Map<String, Object?>> _listMcpReputations(
    Map<String, Object?> arguments,
  ) async {
    final HarnessMcpReputationLoader? loader = _mcpReputationLoader;
    if (loader == null) {
      throw StateError('当前 Harness 尚未连接 MCP 信誉存储');
    }
    return loader();
  }

  Future<Map<String, Object?>> _rateMcpTool(
    Map<String, Object?> arguments,
  ) async {
    final HarnessMcpReputationRater? rater = _mcpReputationRater;
    if (rater == null) {
      throw StateError('当前 Harness 尚未连接 MCP 信誉存储');
    }
    final String tier = '${arguments['tier'] ?? ''}'.trim();
    final String instanceId = '${arguments['instanceId'] ?? ''}'.trim();
    final String toolName = '${arguments['toolName'] ?? ''}'.trim();
    final int rating = _integer(arguments['rating'], -1);
    if (tier.isEmpty ||
        instanceId.isEmpty ||
        toolName.isEmpty ||
        rating < 0 ||
      ...
      throw const FormatException(
        'tier、instanceId、toolName 必填，rating 必须为 0–5 整数',
      );
    }
    return rater(tier, instanceId, toolName, rating);
  }
  ...
  static const Map<String, Object?> _agentCliProviderIdSchema =
      <String, Object?>{
        'type': 'string',
        'enum': <String>[
          'codex',
        ...
        'description': '智能体 CLI 稳定提供方 ID；必须来自 catalog 返回值',
      };
  static const Map<String, Object?> _agentCliArgumentsSchema =
      <String, Object?>{
        'type': 'array',
        'items': <String, Object?>{'type': 'string'},
        'minItems': 1,
        'maxItems': 128,
        'description': '逐项 argv，不得拼接成 shell 命令，不得包含密钥',
      };
  ...
  static String _targetSummary(String toolId, Map<String, Object?> arguments) {
    if (toolId == mcpToolCallId) {
      return '${arguments['instanceId'] ?? ''} · ${arguments['toolName'] ?? ''}';
    }
    if (toolId == mcpReputationRateId) {
      return '${arguments['toolName'] ?? ''} · ${arguments['rating'] ?? ''}/5';
    }
