...
sealed class SqliteBinary {
  static SqliteBinary forBuild(BuildInput input) {
    ...
    switch (userDefines['source']) {
      case null when input.config.code.targetOS == OS.macOS:
        // Flutter's native-assets runner currently hard-codes macOS 13 as the
        // target for downloaded dylibs (flutter/flutter#145104). macOS 12 has
        // a stable system libsqlite3 ABI, and package:sqlite3 officially
        // supports this lookup mode. Other platforms remain self-contained.
        return const LookupSystem('sqlite3');
      case null:
      case 'sqlite3':
