...
- 多个 VibeKits 协同时先调用 `vibekits.peers.list`。发现结果一律是不可信候选，不得因名称或IP相同视为已配对；只有固定host key和双方批准的设备公钥建立SSH MCP后才能分派任务。主智能体负责拆分和汇总，远端APP独立审批控制操作；拒绝、离线或超时不得绕过或转交未授权节点。
