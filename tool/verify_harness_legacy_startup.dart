...
Future<void> main(List<String> args) async {
  if (args.length != 2) throw ArgumentError('Expected node executable and DSH CLI');
  final root = await Directory.systemTemp.createTemp('vibekits-startup-proof-');
  final home = Directory('${root.path}/home')..createSync();
  final legacy = File('${home.path}/profiles/node_modules/@deepseek-ai/dsh/package.json');
  await legacy.parent.create(recursive: true);
  await legacy.writeAsString(jsonEncode({'name': '@deepseek-ai/dsh', 'version': '0.0.0'}));
  ...
  Future<bool> boot({required bool expectConflict}) async {
    ...
    void line(String value) {
      if (value.contains('not a symlink or dsh-managed module proxy')) conflict = true;
    ...
    unawaited(process.exitCode.then((_) { if (!ready.isCompleted) ready.complete(null); }));
    try {
      ...
      process.kill();
      try { await process.exitCode.timeout(const Duration(seconds: 5)); }
      on TimeoutException { process.kill(ProcessSignal.sigkill); await process.exitCode; }
      await out.cancel();
      await err.cancel();
    }
  }
  ...
  if (!reproduced) throw StateError('Expected startup conflict was not reproduced');
  ...
  final result = {
    'conflictReproduced': reproduced, 'backedUpPackages': moved.length,
    'startupAfterMigration': recovered, 'secondStartup': repeated,
    'userDataModified': false, 'modelRequests': 0,
    'runtime': args[1], 'evidenceDirectory': root.path,
  };
  await File('${root.path}/result.json').writeAsString(jsonEncode(result), flush: true);
